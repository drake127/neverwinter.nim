...
CExoString CScriptCompiler::TlkToString(STRREF nStrRef) const
{
    static const std::unordered_map<STRREF, const char*> s_lut = {
    ...
    if (s_lut.find(nStrRef) != s_lut.end())
    {
        return s_lut.at(nStrRef);
    }

    return CExoString::F("ERROR: %d", nStrRef);
}
